...
// ─── Component ────────────────────────────────────────────────────────────────
export default function CivilianSOSPage() {
    ...
    <>
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#0d131f]/80 backdrop-blur-xl flex justify-between items-center px-6 h-16 border-b border-white/5 shadow-[0_0_20px_rgba(255,179,173,0.06)]">
        <div className="flex items-center gap-4">
          ...
          <span className="bg-surface-container-high text-primary text-[10px] font-black px-2 py-0.5 rounded-sm border border-primary/20 tracking-widest hidden md:inline">CIVILIAN PORTAL</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="hidden sm:block text-xs font-bold text-on-surface">{user?.name || "Profile"}</span>
          </button>
        </div>
      </nav>
              ...
              return (
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-3 pt-2 border-t border-outline-variant/10">
                    <button
                      type="button"
                      onClick={() => { setDetailId(null); router.push("/civilian/dashboard"); }}
